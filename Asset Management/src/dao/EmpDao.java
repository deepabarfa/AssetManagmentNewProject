...
public int rejectReq(Request r) {
	...
	public int supdateEmp(Employee c) {
		...
		if(c.getEid2()!=null)
		{
		String hql="update Employee set address=:a,email=:b,mobile=:c where eid2=:d";
	    Query q=ss.createQuery(hql);
	    q.setString("a",c.getAddress());
	    q.setString("b",c.getEmail());
	    q.setLong("c",c.getMobile());
	    q.setString("d",c.getEid2());
	    Transaction tt=ss.beginTransaction();
	     x=q.executeUpdate();
		    tt.commit();
		}
...
public int approveEmpTransferReq(UserAssetRequest r) {
...
public int rejectEmpTransferReq(UserAssetRequest r) {
...
public int approveAssetTransferReqFromEmployee(String fromemp, int requestid,String assetid) 
{	int z=0;
	int x=0;
	int status=4;
	AssetController ac=new AssetController();
	...
	if(x!=0)
	{
		AssetController acc=new AssetController();
		Criteria crit=ss.createCriteria(UserAssetRequest.class);
		crit.add(Restrictions.eq("assetid",assetid));
		System.out.println("122ew"+assetid);
		ArrayList<UserAssetRequest> list = (ArrayList<UserAssetRequest>) crit.list();
		UserAssetRequest b=(UserAssetRequest)list.get(0);
		String toemp1=b.getToemp();
		String fromemp1=b.getFromemp();
		System.out.println("hiiiiiii onnnnnn"+toemp1+fromemp1);
		if(!list.isEmpty())
		{
			AssetController ac1=new AssetController();
			Criteria crit1=ss.createCriteria(AllotedAsset.class);
			crit1.add(Restrictions.eq("assetid",assetid));
			System.out.println("assetid"+assetid);
			ArrayList<AllotedAsset> list1 = (ArrayList<AllotedAsset>) crit1.list();
			AllotedAsset a=(AllotedAsset)list1.get(0);
			String empid=a.getEid1();
			String emid=a.getMid1();
			String mngid=a.getMid2();
			System.out.println("hiiii byyyyy"+empid+"emid"+emid+"mngid"+mngid);
			if(toemp1.equals(empid))
				{
				
					AssetController ac2=new AssetController();
					System.out.println("hiiiiiiii");
					if(fromemp!=null)
					{
					String hql1="update AllotedAsset set eid1=:a, mid1=:b, mid2=:c where assetid=:d";
					Query q1=ss.createQuery(hql1);
					q1.setString("a","na");
					q1.setString("b","na");
					q1.setString("c",fromemp1);
					q1.setString("d",assetid);
					Transaction tt2=ss.beginTransaction();
					z=q1.executeUpdate();
					tt2.commit();
					}
				}
				if(toemp1.equals(mngid))
				{
					
					AssetController ac3=new AssetController();
					if(fromemp!=null)
					{
					String hql2="update AllotedAsset set mid2=:a where assetid=:b";
					Query q2=ss.createQuery(hql2);
					q2.setString("a",fromemp1);
					q2.setString("b",assetid);

					Transaction tt3=ss.beginTransaction();
					z=q2.executeUpdate();
					tt3.commit();
...
public int rejectAssetTransferReqFromEmployee(String fromemp, int requestid) 
{
	int x=0;
	int status=5;
	AssetController ac=new AssetController();
...
public int managerprofileupdate(String user1, String name, String email, String address, long mobile) {
	int x=0;
	AssetController ac=new AssetController();
	Session ss= ac.session();
    String hql="update Employee set name=:b, email=:c, mobile=:d, address=:e where mid2=:a and designation='Manager'";
    Query q=ss.createQuery(hql);
    q.setString("b",name);
    q.setString("c",email);
    q.setLong("d",mobile);
    q.setString("e",address);
    q.setString("a",user1);
    Transaction tt=ss.beginTransaction();
     x=q.executeUpdate();
...
public ArrayList<AllotedAsset> alloteassetemployee(String user1) 
	{
		AssetController ac=new AssetController();
		Session ss= ac.session();
		Criteria crit = ss.createCriteria(AllotedAsset.class);
...
public ArrayList<AllotedAsset> viewmallotedasset() 
{
	AssetController ac=new AssetController();
	Session ss= ac.session();
	Criteria crit = ss.createCriteria(AllotedAsset.class);
...
public ArrayList<UserAssetRequest> viewtransferrequestfromemployee(String user1) 
{
	int status=2;
	AssetController ac=new AssetController();
	Session ss= ac.session();
