...
//	protected ModelAndView handleRequestInternal(HttpServletRequest arg0, HttpServletResponse arg1) throws Exception {
  	...
  	public ModelAndView Adminlogin()
  {
	...
	int y=0;
	        ...
	        mv.addObject("msg1","Login failed, please try again");
	        }
	        return mv;
		}
...
//         sendMail(email,subject,message);
   ...
   @RequestMapping("/UpdateViewSupP")//Employee can update his own profile 
   public ModelAndView UpdateViewSupP(@ModelAttribute("Employee")Employee c,HttpServletRequest request)
	{
       ModelAndView mv=null;
    	   	 HttpSession ss=request.getSession();
    	     	  String y=(String)ss.getAttribute("user");
    	   	...
    	   	EmpDao ed=new EmpDao();
    	   	ArrayList<Employee>list=ed.viewSProfile(y);//calling function to view support profile 
    	   	mv=new ModelAndView("UpdateViewSupP");
    	   	mv.addObject("LIST", list);
    	   	
    	   		return mv;

	}
   @RequestMapping("/SUpdateEmp")//Employee can update his own profile 
   public ModelAndView SUpdateEmp(@ModelAttribute("Employee")Employee c,HttpServletRequest request)
	{     
      ModelAndView mv=null;
      EmpDao l=new EmpDao();
      int z=l.supdateEmp(c);//calling function to update Employee's own profile
      if(z==1)
      {
    	  ...
    	  HttpSession ss=request.getSession();
    	   	...
    	   	mv=new ModelAndView("viewSupProfile");
    	   	mv.addObject("LIST", list);
    	   	mv.addObject("msg","Profile Successfully Updated");
    	   	   }
		return mv;
...
//============================================Manager Module===================================================
  ...
  @RequestMapping("/viewa")//view profile
  public ModelAndView viewa(HttpServletRequest request)//request for  session
  {
 		ModelAndView mv=null;
 		EmpDao ed=new EmpDao();
 		HttpSession ss=request.getSession();
  ...
  public ModelAndView opreation(HttpServletRequest request,@RequestParam String opreation)
  {
	  ModelAndView mv=null;
	  if(opreation.equalsIgnoreCase("update profile"))
	  {
 		EmpDao ed=new EmpDao();
 		HttpSession ss=request.getSession();
 		String user1=(String)ss.getAttribute("user");
 		System.out.println(user1);
 		ArrayList<Employee> list=ed.viewmanagerprofile(user1);
 		mv=new ModelAndView("Managerupdateprofile");
 		mv.addObject("LIST", list);
 		
	  }
	  else if(opreation.equalsIgnoreCase("changePassword"))
	  {
...
public ModelAndView managerprofileupdate(HttpServletRequest request,@RequestParam String name,@RequestParam String email,@RequestParam String address,@RequestParam long mobile)
{
	ModelAndView mv=null;
	HttpSession ss=request.getSession();
	String user1=(String)ss.getAttribute("user");
	EmpDao ed=new EmpDao();
	int y=ed.managerprofileupdate(user1,name,email,address,mobile);
	if(y!=0)
	{
		EmpDao ed1=new EmpDao();
		HttpSession sss=request.getSession();
		String user11=(String)sss.getAttribute("user");
		ArrayList<Employee> list=ed1.viewmanagerprofile(user11);
		mv=new ModelAndView("managerprofile");
		mv.addObject("LIST", list);
			mv.addObject("msg", "Profile successfully changed");
		...
		else
		{
			EmpDao ed1=new EmpDao();
	 		...
	 		mv=new ModelAndView("managerprofile");
	 		mv.addObject("LIST", list);
	 		mv.addObject("msg","profile not updated");
...
//	 	      sendMail(email,subject,message);
	  ...
	  @RequestMapping("/ETransferAssetFromOther")//employee can view his asset that his allocated 
	 	protected ModelAndView viewETransferRequestFromOther(@ModelAttribute("UserAssetRequest") UserAssetRequest u,HttpServletRequest request,@RequestParam String op) {  
		  ModelAndView mv=null;
...
//	 	  String y=(String)ss.getAttribute("user");
	      EmpDao ed=new EmpDao();
	      int y1=ed.approveEmpTransferReq(u);
	      if(y1==1)
	      {
	    	  HttpSession ss=request.getSession();
		 	  String y=(String)ss.getAttribute("user");
	 	  ...
	 	  {
	 		 EmpDao ed1=new EmpDao();
		      int y2=ed1.rejectEmpTransferReq(u);
		      if(y2==1)
		      {
		    	  HttpSession ss=request.getSession();
			 	  String y=(String)ss.getAttribute("user");
			      ...
			      mv=new ModelAndView("ViewAssetRequestFromOther");
			      mv.addObject("User", list);
			      mv.addObject("msg2","Request rejected ");
		 	  }
	  ...
	  @RequestMapping("/assetallotedemployee")
	 	protected ModelAndView allotedassetemployee(HttpServletRequest request,HttpServletResponse response)
	 	{
			ModelAndView mv=null;
	 		HttpSession ss=request.getSession();
	 		String user1=(String)ss.getAttribute("user");
	 		System.out.println(user1);
	 		EmpDao ed=new EmpDao();
	 		ArrayList<AllotedAsset> list=ed.alloteassetemployee(user1);
	 		mv=new ModelAndView("allotedEmployeeasset");
	 	    mv.addObject("LIST", list);
	 		
	 		return mv;
	 	}	  
	  
	  @RequestMapping("/viewallallotedasset")
		protected ModelAndView AssetTransferRequ(HttpServletRequest request,HttpServletResponse response)
	 	{
			ModelAndView mv=null;
	 		EmpDao ed=new EmpDao();
	 		ArrayList<AllotedAsset> list=ed.viewmallotedasset();
	 		mv=new ModelAndView("Mviewallallotedasset");
	 	    mv.addObject("LIST", list);
	 		
	 		return mv;
	 	}
	  
	  @RequestMapping("/viewtransferrequestfromemployee")
		protected ModelAndView viewtransferrequestfromemployee(HttpServletRequest request,HttpServletResponse response)
	 	{
			ModelAndView mv=null;
			HttpSession ss=request.getSession();
	 		String user1=(String)ss.getAttribute("user");
	 		EmpDao ed=new EmpDao();
	 		ArrayList<UserAssetRequest> list=ed.viewtransferrequestfromemployee(user1);
	 		mv=new ModelAndView("viewrequestfromemployee");
	 	    mv.addObject("LIST", list);
	  ...
	  @RequestMapping("/MTransferRequest")
		  protected ModelAndView MTransferRequest(@ModelAttribute("UserAssetRequest") UserAssetRequest u,HttpServletRequest request ) {
		 	  ModelAndView mv=null;
		 	 RequestDao rd=new RequestDao();
		 	HttpSession ss=request.getSession();
			  String fromemp=(String) ss.getAttribute("user");
...
//		 	      sendMail(email,subject,message);
	  ...
	  @RequestMapping("/Requestfromemployee")
		protected ModelAndView Requestfromemployee(HttpServletRequest request,@RequestParam String op,@RequestParam String assetid,@RequestParam String fromemp,@RequestParam int requestid)
	 	{
		  ModelAndView mv=null;
		  if(op.equalsIgnoreCase("Approve"))
	 	  {
	      EmpDao ed=new EmpDao();
	      int y1=ed.approveAssetTransferReqFromEmployee(fromemp,requestid,assetid);
	      System.out.println(" y1 in request from emp approve "+y1);
	      if(y1!=0)
	      {
	    		HttpSession ss=request.getSession();
		 		String user1=(String)ss.getAttribute("user");
		 		EmpDao ed2=new EmpDao();
		 		ArrayList<UserAssetRequest> list=ed2.viewtransferrequestfromemployee(user1);
		 		mv=new ModelAndView("viewrequestfromemployee");
			 	   mv.addObject("msg1","Request Approveed  by Manager");
					 	    mv.addObject("LIST", list);
		 }
	 	  }
	      else if(op.equalsIgnoreCase("Reject"))
	 	  {
	 		 EmpDao ed=new EmpDao();
		      int y2=ed.rejectAssetTransferReqFromEmployee(fromemp,requestid);
		      if(y2==1)
		      {
		    	  HttpSession ss=request.getSession();
			 		String user1=(String)ss.getAttribute("user");
			 		EmpDao ed3=new EmpDao();
			 		ArrayList<UserAssetRequest> list=ed3.viewtransferrequestfromemployee(user1);
					mv=new ModelAndView("viewrequestfromemployee");
				 	   mv.addObject("msg2","Request rejected  by Manager");
						 	    mv.addObject("LIST", list);
